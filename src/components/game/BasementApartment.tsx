...
import { DebugDisplay } from '@/components/game/DebugDisplay'; // Changed to named import
import SceneObjects, { SceneObjectConfig } from '@/components/game/SceneObjects'; 
import { roomStyle, gameTitleStyle } from '@/app/page.styles'; // Corrected import path
import FridgeMenu from '@/components/game/FridgeMenu';
import DoorMenu from '@/components/game/DoorMenu'; 
import StoreMenu from '@/components/game/StoreMenu'; 
import EnemyInfoMenu from '@/components/game/EnemyInfoMenu'; 

interface BasementApartmentProps {
...
// --- BasementApartment Component ---
const BasementApartment: React.FC<BasementApartmentProps> = ({ onEnterFightRoom }) => {
  // Game State
  ...
  const [isEnemyInfoMenuOpen, setIsEnemyInfoMenuOpen] = useState(false); 

  const [fridgeInventory, setFridgeInventory] = useState<FridgeSlot[]>([
    { item: null, count: 0 },
    { item: null, count: 0 },
    { item: null, count: 0 },
    { item: null, count: 0 },
  ...
  const handleFightOption = () => {
    ...
    // --- Enemy/Fight Related Handlers ---
      const handleFight = () => {
          setIsEnemyInfoMenuOpen(false);
          onEnterFightRoom(); // Call the prop to change room
          setDoorTarget(null); 
      };
  ...
  return (
    <div style={roomStyle} onClick={handleBackgroundClick}>
      ...
      <DebugDisplay
        ...
        onEnterFightRoom={onEnterFightRoom}
      />
...
export default BasementApartment;